...
export const obtenerTiposPersonal = async (req: Request, res: Response) => {
  try {
    const result = await query('SELECT * FROM tipos_personal ORDER BY id');
    res.json({
      success: true,
      data: result.rows
    });
...
export const obtenerEspecialidades = async (req: Request, res: Response) => {
  try {
    const result = await query('SELECT * FROM especialidades ORDER BY id');
    res.json({
      success: true,
      data: result.rows
    });
